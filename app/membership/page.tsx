    ...
    <>
      ...
      <section className="border-b border-white/10 bg-surface px-4 py-16 md:px-10 md:py-24">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="mt-6 font-body text-base text-off-white/60 max-w-xl leading-relaxed">
      ...
      {/* ── 2. Schedule ─────────────────────────────────────────────────── */}
      <section className="bg-base py-16 px-4 md:px-10 border-b border-white/10">
        <div className="max-w-7xl mx-auto">
              ...
              return (
                <div key={day.day}>
                  <button
                    ...
                    className="w-full flex items-center justify-between px-0 py-4 text-left"
                  >
                    <span className="font-display text-xl text-off-white uppercase">
                      {day.day}
                    </span>
                    <span className="font-body text-off-white/40 text-lg">
                      {isOpen ? "−" : "+"}
                    </span>
                  </button>
                  {isOpen && (
                    <div className="pb-4 space-y-3">
                      {day.classes.map((cls, i) => (
                        <div
                          key={i}
                          className="bg-surface px-4 py-3 border-l-2 border-accent"
                        >
      ...
      {/* ── 3. Pricing ──────────────────────────────────────────────────── */}
      <section className="bg-surface py-16 px-4 md:px-10 border-b border-white/10">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-display text-3xl md:text-5xl text-off-white mb-10">
            Pricing
          </h2>
              ...
              >
                <p
                  className={`font-display text-sm uppercase tracking-widest ${
                    plan.featured ? "text-off-white/70" : "text-off-white/40"
                  }`}
                >
                  {plan.name}
                </p>
                <div className="mt-4 mb-1">
                  <span
                    className={`font-display text-5xl leading-none ${
                      plan.featured ? "text-off-white" : "text-off-white"
                    }`}
                  >
                    {plan.price}
                  </span>
                ...
                <p
                  className={`font-body text-xs uppercase tracking-widest mb-8 ${
                    plan.featured ? "text-off-white/60" : "text-off-white/35"
                  }`}
                >
                  {plan.period}
                </p>
                    ...
                    <li key={f} className="flex items-start gap-2">
                      <span
                        className={`mt-0.5 shrink-0 ${
                          plan.featured ? "text-off-white" : "text-accent"
                        }`}
                      >
                        ✓
                      </span>
                      <span
                        className={`font-body text-sm ${
                          plan.featured ? "text-off-white/80" : "text-off-white/60"
                        }`}
                      >
                        {f}
                      </span>
                ...
                <a
                  ...
                  className={`mt-8 inline-block text-center py-3 font-body text-sm uppercase tracking-widest transition-colors ${
                    plan.featured
                      ? "bg-off-white text-base hover:bg-off-white/90"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/10">
            {membership.coaches.map((coach) => (
              <div key={coach.name} className="bg-base">
                <ImagePlaceholder
                  label={coach.imageLabel}
                  aspectRatio="aspect-[3/4]"
                />
                <div className="px-4 py-5">
                  <p className="font-display text-lg text-off-white uppercase leading-none">
                    {coach.name}
                  </p>
              ...
              return (
                <div key={i}>
                  ...
                  >
                    ...
                    <span className="shrink-0 font-body text-off-white/40 text-xl mt-0.5">
                      {isOpen ? "−" : "+"}
                    </span>
                  </button>
                  {isOpen && (
