...
export default function MembershipPage() {
  ...
  return (
    <>
      {/* ── 1. Hero ─────────────────────────────────────────────────────── */}
      <section id="hero" className="relative overflow-hidden border-b border-white/10 min-h-[55vh] flex items-end">
        <Image
          src={membership.hero.image}
          alt={membership.hero.imageAlt}
          fill
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-black/40 to-black/92 z-[3]" />
        <div className="relative z-[4] max-w-7xl mx-auto w-full px-4 pb-14 md:px-10 md:pb-20">
      ...
      <FadeUp>
        <section id="pricing" className="bg-surface py-14 md:py-16 px-4 md:px-10 border-b border-white/10">
          <div className="max-w-7xl mx-auto">
            <h2 className="font-display text-3xl md:text-5xl text-off-white mb-10">Pricing</h2>
              ...
              {membership.plans.map((plan) => (
                ...
                >
                  ...
                  <a
                    ...
                    className={`mt-8 inline-block text-center py-3.5 font-body text-xs uppercase tracking-widest transition-colors ${
                      plan.featured
                        ? "bg-off-white text-[#0A0A0B] hover:bg-off-white/90"
                        : "border border-white/20 text-off-white hover:border-white/40 hover:bg-white/5"
                    }`}
                  >
                    {plan.cta}
                  </a>
      ...
      <FadeUp>
        <section id="faq" className="bg-surface py-14 md:py-16 px-4 md:px-10 border-b border-white/10">
          <div className="max-w-3xl mx-auto">
            <h2 className="font-display text-3xl md:text-5xl text-off-white mb-10">FAQ</h2>
      ...
      {/* ── 6. CTA over image ───────────────────────────────────────────── */}
      <section id="join" className="relative py-20 md:py-28 px-4 md:px-10 overflow-hidden">
        <Image
          src={membership.ctaBand.image}
          alt={membership.ctaBand.imageAlt}
          fill
          className="object-cover object-center"
          sizes="100vw"
        />
        <ImageGrain opacity={0.04} />
        {/* Red band covers image with 85% opacity — brand colour, not just dark */}
        <div className="absolute inset-0 bg-accent/85 z-[3]" />
        <div className="relative z-[4] max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-8">
          <div>
